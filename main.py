...
class Person:
    ...
    def saveMyFile(self):
        if debugMode == True:
            print("Func- log_user_data(" + self.firstName + ")" + "\n")
        ...
        isExist = os.path.exists(
            logpath + self.firstName + "/" + date + "_" + self.firstName + ".csv")

        if debugMode == True:
            print(logpath + self.firstName + "/" + date +
                  "_" + self.firstName + ".csv" + "\n")
        if debugMode == True:
            print(str(isExist) + "\n")
        ...
        if isExist == False:
            ...
            filename = logpath + self.firstName + "/" + date + "_" + self.firstName + ".csv"
            fields = ['Name', 'Coordinates', 'Time', 'Battery Level', "Currently Charging?",
                      "In Transit?", "Current Speed", "Start Timestamp", "End Timestamp"]
        ...
        data = [self.firstName, self.lastName, self.latitude, self.longitude, self.batteryLevel,
                self.batteryCharging, self.inTransit, self.speed, self.startTimestamp, self.since]

        current_datetime = datetime.now(tz).strftime("%I:%M %p")

        if data[0] == "Life-360_Error":
            if debugMode == True:
                print("Life-360 Couldn't Authenticate" + "\n")
            pass
            ...
            with open(filename, 'a') as file:
                ...
                row = [data[0] + " " + data[1], data[2] + "," + data[3],
                       current_datetime, data[4] + "%", data[5], data[6], data[7], data[8], data[9]]
                if debugMode == True:
...
def collect_data(save):
    # fortnite meme
    personData = Person("Tomato", "Head", "Tomato", "Town", "3 mini sheilds", "100 sheild",
                        "dropping in tilted", "battlebus speed", "Storm", "Closing")
    avatarList = []
    emailList = []
    if api.authenticate():
        ...
        personDict = {}
        for mem in members:
            personDict.setdefault(mem["firstName"], []).append([
                mem["firstName"], mem["lastName"], mem["id"], mem["loginEmail"], mem["avatar"], mem["loginPhone"]])
        ...
        for name, data in personDict.items():
            ...
            if info['location'] != None:
                ...
                personData.startTimestamp = datetime.fromtimestamp(
                    int(info['location']['startTimestamp']))
                # YOU NEED TO USE strftime('%Y-%m-%d %H:%M:%S') TO USE THESE DATETIME OBJECTs!!!!
                personData.since = format_date(
                    datetime.fromtimestamp(int(info['location']['since'])))
                ...
                if debugMode == True:
                    print("\tBattery level:",
                          info['location']['battery'] + "%")
                personData.batteryLevel = info['location']['battery']
                if debugMode == True:
                    print("\tCharging?", info['location']['charge'])
